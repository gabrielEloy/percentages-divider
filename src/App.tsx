  ...
  return (
    <div className="App">
      <input
        ...
        onChange={(e) => setCurrentItemLabel(e.currentTarget.value)}
      />
      <button onClick={handleInsertItem}>insert new item</button>

      <span>splittedValue</span>
      <input
        value={totalValueLimit}
        onBlur={updateItemsOnTotalValueChange}
        onChange={(e) => setTotalValueLimit(Number(e.currentTarget.value))}
      ></input>
          ...
          {items.map((item) => (
            <li key={item.id}>
              ...
              <input
                ...
                min={0}
                max={totalValueLimit}
                onChange={(e) => onUpdateItem(item.id, e.currentTarget.value)}
                value={item.value}
              />
